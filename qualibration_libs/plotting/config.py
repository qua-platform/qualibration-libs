...
def with_palette_param(func: Callable) -> Callable:
    """Decorator that adds palette parameter support to plotting functions.
    
    This decorator allows plotting functions to accept a 'palette' parameter
    that temporarily sets the global palette during execution and restores it afterward.
    
    The decorated function will accept an optional 'palette' parameter that can be:
    - A string name of a predefined palette (e.g., 'viridis', 'plasma', 'tab10')
    - A tuple/list of color strings (e.g., ('#ff0000', '#00ff00', '#0000ff'))
    ...
    Examples:
        ...
        >>> fig = plot_data(data, palette='viridis')
        
        >>> # Use custom colors
        >>> fig = plot_data(data, palette=['#ff0000', '#00ff00', '#0000ff'])
        
        >>> # Use current global palette
        >>> fig = plot_data(data)
    
    Note:
        This decorator automatically handles palette restoration, so it's safe to use
        with nested calls and exceptions.
    """
    @wraps(func)
    def wrapper(*args, **kwargs):
        global CURRENT_PALETTE
        
        # Extract palette parameter if provided
        palette = kwargs.pop('palette', None)
        
        if palette is None:
            # No palette specified, use current global palette
            return func(*args, **kwargs)
