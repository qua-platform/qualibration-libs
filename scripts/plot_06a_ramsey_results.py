...
# Ensure we can import the plotting utilities exactly like the node 06a
if SUPERCONDUCTING_DIR not in sys.path:
    sys.path.append(SUPERCONDUCTING_DIR)

from calibration_utils.ramsey.plotting import (  # noqa: E402
...
def main():
    parser = argparse.ArgumentParser(description="Plot 06a Ramsey results.")
    parser.add_argument(
        "--date-dir",
    ...
    for folder in candidates:
        ...
        try:
            fig.canvas.manager.set_window_title(f"Ramsey: {folder.name}")
        except Exception:
            pass
